...
> **Backlog (последняя очередь):** см. [FEATURES_BACKLOG.md](FEATURES_BACKLOG.md) — **ачивки у аватара** и **комментарии** берутся в работу **в самом конце**, после основного плана. Для комментариев: **обязательны** администрирование (модерация) и **одобрение HR до публикации** — см. backlog.
...
- Флаг `visible_for_candidates` на контенте/тестах (что доступно кандидатам).
...
- **11.7.1 Регистрация кандидата + телефон**
  ...
  - для `employee` — телефон желателен, можно добить позже через профиль
- **11.7.2 Ограниченный интерфейс кандидата**
  - минимальная навигация (1–2 раздела)
